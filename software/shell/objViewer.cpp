...
int objvLoadOBJFS( char *fileName, float scaleFactor, tgfBitmap *texture )
{
	...
	points 		= (tgfPoint3D*)osAlloc( sizeof( tgfPoint3D ) * ( objNumPoints + 1 ), OS_ALLOC_MEMF_FAST );
	triangles 	= (tgfTriangle3D*)osAlloc( sizeof( tgfTriangle3D ) * ( objNumTriangles + 1 ), OS_ALLOC_MEMF_FAST );

	if( objNumTexturePoints > 0 )
	{
		tx = (float*)osAlloc( sizeof( float ) * ( objNumTexturePoints + 1 ), OS_ALLOC_MEMF_FAST );
		ty = (float*)osAlloc( sizeof( float ) * ( objNumTexturePoints + 1 ), OS_ALLOC_MEMF_FAST );
	}
